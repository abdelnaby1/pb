...
const HoriznotalProducts = ({onClose}:IProps) => {
  ...
  const { register, formState: { errors }, handleSubmit } = useForm<IProducts>({resolver: yupResolver(addProductsSchema)})

  const onProductsSubmit: SubmitHandler<IProducts> = async (data) => {
           try {
            setIsLoading(true)
            const widgetsRef = doc(firestore, `${userData.uid}`, "widgets");
            await setDoc(widgetsRef,{data: arrayUnion({...data,component_type:"Horiznotal_Products"})},{merge:true})
  ...
  return (
      <div className="mt-5">
        <h2 className="text-center mb-4 text-3xl font-semibold">Add Horiznotal Products Widget</h2>
